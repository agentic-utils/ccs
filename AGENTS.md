...
- `parseAppended()` / `consumeLines()` - incremental parse: seek to `parsedBytes` (end of the last complete line) and parse only appended lines. A line caught mid-write isn't applied, so parsing resumes at it; full reparse only if the file shrank (prune) or an unterminated last line *was* applied as a record (`tailApplied`). Search text is rebuilt with `searchTextOf` (identical to a full parse) only when something it covers changed
- `keepNewer()` - a full scan never replaces a conversation the live tick already parsed further (larger `Size`), so a slow scan can't roll a live session back
- `runBounded()` - every ps/tmux/osascript call has a deadline; Enter on a live session focuses it via a `tea.Cmd` (`focusDoneMsg`), never on the UI goroutine
