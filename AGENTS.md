...
- `latestRelease()` / `newerVersion()` / `chooseUpgrader()` - Self-update: at startup and hourly, read the newest tag from the `github.com/.../releases/latest` redirect (not the REST API, whose 60/h unauthenticated limit shared IPs exhaust); if newer, a popup over the preview offers Enter = update and re-exec, Esc = later (for that version, this session). Keys are ignored for `updateKeyGrace` after it opens. How it updates depends on where the binary lives: a Homebrew keg (`/Cellar/`) only ever via `brew update && brew upgrade ccs`, never overwritten; `/nix/` notify only; anything else `replaceBinary()` downloads the release archive, verifies it against `checksums.txt`, and atomically renames the new binary over the old. `dev` builds never check
