...
- `runBounded()` - every ps/tmux/osascript call has a deadline (and `WaitDelay`, so an inherited pipe can't outlive it)
- `resumeCmd()` - Enter/Ctrl+F run off the UI goroutine: fresh liveness check, then focus (`focusDoneMsg`), a new tab/window, or exec in place (`resumeDoneMsg`). A tab-open timeout is reported, never followed by a second launch; one open at a time. tmux `-c` gets `#` escaped (tmux expands `#(cmd)` there)
